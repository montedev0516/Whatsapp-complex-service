...
class SmsController extends Controller
{
    public function VueMessage(Request $request)
    {
        ...
        try {
            ...
            Log::debug('Twilio data:', ['sid' => $sid, 'token' => $token, 'body' => $message_decode]);
    ...
    public function MediaMessage(Request $request)
    {
        try {
            if ($request->hasFile('file')) {
                ...
                // $fileUrl = asset('/storage/' . $filePath); // Generate URL to access file
                $fileUrl = "https://relaxing-dane-lively.ngrok-free.app/storage/" . $filePath; // Generate URL to access file
                
                Log::debug('media data:', ['url' => $fileUrl, "num" => $request->input('PhoneNumber') ]);
                ...
                $twilio->messages
                  ->create("whatsapp:" . $request->input('PhoneNumber'), [
                    "mediaUrl" => [$fileUrl],
                    "from" => "whatsapp:+14155238886",
                ]);
    ...
    {
        $imageExtensions = ['png', 'jpg', 'jpeg', 'gif', 'jfif', 'webm'];
        $videoExtensions = ['mp4', 'avi', 'mov'];
        $audioExtensions = ['mp3', 'wav', 'aac', 'ogg'];
        $documentExtensions = ['doc', 'docx', 'pdf', 'txt', 'ppt'];
    ...
    public function receiveMessage(Request $request)
    {
        ...
        Log::debug('recieve data:', ['phoneNumber' => $phoneNumber, 'body' => $body]);
        if ($numMedia > 0) {
    ...
    private function downloadAndStoreMedia($phoneNumber, $mediaUrl, $index)
    {
        ...
        $response = Http::withBasicAuth(getenv('TWILIO_SID'), getenv('TWILIO_AUTH_TOKEN'))->get($mediaUrl);
        ...
        if ($response->successful()) {
            ...
            $storagePath = 'storage/uploads/' . $filename; // Path inside the public directory
            ...
            if ($extension == 'jpg' || $extension == 'png' || $extension == 'gif' || $extension == 'jifi' || $extension == 'webp') {
                $type = 'image';
            } elseif ($extension == 'mp4' || $extension == 'mov' || $extension == 'avi' || $extension == '3gp' ) {
                $type = 'video';
            } elseif ($extension == 'pdf' || $extension == 'doc' || $extension == 'docx' || $extension == 'ppt' || $extension == 'txt') {
                $type = 'document';
